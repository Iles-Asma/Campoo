...
function _renderUnversionedPostinstall(sdkVersion) {
  ...
  const sdkMajorVersion = (0, _ExponentTools().parseSdkMajorVersion)(sdkVersion);
  const podNameExpression = sdkMajorVersion < 33 ? 'target.pod_name' : 'pod_name';
  const targetExpression = sdkMajorVersion < 33 ? 'target' : 'target_installation_result'; // SDK41 drops support for iOS 10.0
...
function _renderUnversionedUniversalModulesDependencies(universalModules, universalModulesPath, sdkVersion) {
  ...
  if (sdkMajorVersion >= 33) {
    const excludedUnimodules = ['expo-bluetooth', 'expo-in-app-purchases', 'expo-payments-stripe', 'expo-module-template', 'expo-image'];
    ...
    if (sdkMajorVersion >= 39) {
      expoModulesThatAreNotUnimodules.push(`pod 'EXRandom', path: '${universalModulesPath}/expo-random/ios'`);
    }
