...
async function _saveDefaultIconToPathAsync(context, path) {
  if (context instanceof _StandaloneContext().StandaloneContextUser) {
    ...
    if (data.exp.icon) {
      await (0, _ExponentTools().saveImageToPathAsync)(data.projectPath, data.exp.icon, path);
    } else {
    ...
    if (data.manifest.ios && data.manifest.ios.iconUrl) {
      await (0, _ExponentTools().saveUrlToPathAsync)(data.manifest.ios.iconUrl, path);
    } else if (data.manifest.iconUrl) {
      await (0, _ExponentTools().saveUrlToPathAsync)(data.manifest.iconUrl, path);
    } else {
