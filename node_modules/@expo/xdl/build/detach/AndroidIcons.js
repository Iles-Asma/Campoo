...
async function _resizeIconsAsync(context, resPath, prefix, mdpiSize, filename, url, isDetached) {
  const baseImagePath = _path().default.join(resPath, filename);
  ...
  await Promise.all(Object.entries(iconScales).map(async ([folderSuffix, iconScale]) => {
    ...
    const destinationPath = _path().default.join(resPath, subdirectoryName);
...
async function createAndWriteIconsToPathAsync(context, resPath, isDetached) {
  ...
  if (iconUrl || iconForegroundUrl) {
    ...
    if (iconForegroundUrl) {
      (0, _glob().sync)('**/ic_foreground.png', {
        cwd: resPath,
        absolute: true
      }).forEach(filePath => {
        _fsExtra().default.removeSync(filePath);
  ...
  (0, _glob().sync)('**/shell_notification_icon.png', {
    cwd: resPath,
    absolute: true
  }).forEach(filePath => {
    _fsExtra().default.removeSync(filePath);
  ...
  await _resizeIconsAsync(context, resPath, 'drawable-', 24, 'shell_notification_icon.png', (_notificationIconUrl = notificationIconUrl) !== null && _notificationIconUrl !== void 0 ? _notificationIconUrl : iconUrl, isDetached);
}
