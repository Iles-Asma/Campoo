...
function _convertHTMLToASCII() {
  const data = require("../utils/convertHTMLToASCII");
...
async function assertContractMessagesAsync(context, spinner) {
  ...
  if (Array.isArray(messages) && messages.length) {
    ...
    (0, _log().default)(_chalk().default.yellow.bold('Messages from App Store Connect:'));

    _log().default.newLine();

    for (const message of messages) {
      if (_log().default.isDebug) {
        (0, _log().default)(JSON.stringify(message, null, 2));
      ...
      (0, _log().default)(formatContractMessage(message));
    }
