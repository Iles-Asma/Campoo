...
async function promptUsernameAsync() {
  (0, _log().default)('\u203A Log in to your Apple Developer account to continue'); // Get the email address that was last used and set it as
...
async function promptPasswordAsync({
  ...
  if (cachedPassword) {
    (0, _log().default)(`\u203A Using password for ${username} from your local Keychain`);
    (0, _log().default)(`  ${(0, _TerminalLink().learnMore)('https://docs.expo.io/distribution/security#keychain')}`);
    return cachedPassword;
  } // https://docs.expo.io/distribution/security/#apple-developer-account-credentials


  (0, _log().default)((0, _wrapAnsi().default)(_chalk().default.bold(`\u203A The password is only used to authenticate with Apple and never stored on EAS servers`), process.stdout.columns || 80));
  (0, _log().default)(`  ${(0, _TerminalLink().learnMore)('https://bit.ly/2VtGWhU')}`);
  const {
    password
  } = await (0, _prompts().default)({
    type: 'password',
...
async function cachePasswordAsync({
  ...
  (0, _log().default)(`\u203A Saving Apple ID password to the local Keychain`);
  (0, _log().default)(`  ${(0, _TerminalLink().learnMore)('https://docs.expo.io/distribution/security#keychain')}`);
  const serviceName = getKeychainServiceName(username);
