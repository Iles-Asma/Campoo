...
async function ensureTypeScriptSetupAsync(projectRoot) {
  if (_updateTSConfig().isTypeScriptSetupDisabled) {
    (0, _log().default)(_chalk().default.dim('\u203A Skipping TypeScript verification'));
    return;
  }
...
async function shouldSetupTypeScriptAsync(projectRoot) {
  ...
  const typescriptFile = await (0, _resolveModules().queryFirstProjectTypeScriptFileAsync)(projectRoot);
...
async function getSDKVersionsAsync(projectRoot) {
  ...
  } catch (_unused) {// This is a convenience method and we should avoid making this halt the process.
  }
...
async function ensureRequiredDependenciesAsync(projectRoot, skipPrompt = false) {
  ...
  if (versions === null || versions === void 0 ? void 0 : versions.relatedPackages) {
  ...
  if (!skipPrompt) {
    if (await (0, _prompts().confirmAsync)({
      message: title + ` Would you like to install ${_chalk().default.cyan(readableMissingPackages)}?`,
      initial: true
    })) {
